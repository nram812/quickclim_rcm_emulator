...
@tf.function
def predict_batch_residual_v2(model, unet, latent_vectors, data_batch, orog, he, vegt, model_type, scheduler=None, num_inference_steps=25):
    """
    ...
    if model_type == 'diffusion':
        ...
        if scheduler is not None:
            ...
            for i in tf.range(num_inference_steps - 1):
                ...
                alpha_bar_t_next = tf.reshape(alpha_bar_t_next, (batch_size, 1, 1, 1))
                sqrt_one_minus_alpha_bar_t_next = tf.gather(scheduler.sqrt_one_minus_alpha_bar, t_next)
                sqrt_one_minus_alpha_bar_t_next = tf.reshape(sqrt_one_minus_alpha_bar_t_next, (batch_size, 1, 1, 1))
        ...
        # Fallback for other model types or if no model_type is specified.
        return unet([data_batch, orog], training=False)
